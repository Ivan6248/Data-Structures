...
class Quadtree
{
  ...
  private:
    ...
    void pruneHelper(int tolerance, QuadtreeNode* subRoot);

    bool isPruneable(int tolerance, QuadtreeNode* subRoot, QuadtreeNode* child)const;

    bool isPruneable(int tolerance, QuadtreeNode* subRoot, RGBAPixel temp)const;

    int pruneSizeFinder(int tolerance, QuadtreeNode* subRoot)const;

    int idealPruneHelper(int min, int max,int mid, int numLeaves)const;
