...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    {
        TreeNode* freq = new TreeNode(*it);
        singleQueue.push(freq);
    } //loads all of the leaf nodes into the singleQueue
    while(singleQueue.size() + mergeQueue.size() != 1)
    {
        ...
        int newFreq = firstSmallest->freq.getFrequency() + secondSmallest->freq.getFrequency();
        TreeNode* temp = new TreeNode(newFreq);
        temp->left = firstSmallest;
        temp->right = secondSmallest;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    { 
        ...
        Frequency newFreq(c,0);
        temp = new TreeNode(newFreq);
    }
    else
    {
        temp = new TreeNode(0);
