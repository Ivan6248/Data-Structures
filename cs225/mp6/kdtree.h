...
class KDTree
{
  ...
  private:
    ...
    void build(vector< Point<Dim> > & newPoints, int left, int right, int curDim);

    void quickSelect(vector< Point<Dim> > & newPoints,int left, int right,int pivot,int dimension);

    int partition(vector< Point<Dim> > & newPoints,int low, int high,int pivotIndex,int dimension);

    Point <Dim> findNN(const Point<Dim> & query, int left, int right, int dimension) const;
