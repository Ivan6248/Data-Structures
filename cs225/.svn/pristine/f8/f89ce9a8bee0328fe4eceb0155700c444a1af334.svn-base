                                         ...
                                         int centerX, int centerY)
{
     ...
     */
    inFadeColor=fadeColor1;
    outFadeColor=fadeColor2;
    newRadius=radius;
    newCenterX=centerX;
    newCenterY=centerY;
}
...
RGBAPixel gradientColorPicker::operator()(int x, int y)
{
     ...
     */
    int d = abs(newCenterX-x)+abs(newCenterY-y);
    if(d<newRadius){
        color.red=inFadeColor.red-floor((d*inFadeColor.red)/newRadius)+floor((d*outFadeColor.red)/newRadius);
        color.green=inFadeColor.green-floor((d*inFadeColor.green)/newRadius)+floor((d*outFadeColor.green)/newRadius);
        color.blue=inFadeColor.blue-floor((d*inFadeColor.blue)/newRadius)+floor((d*outFadeColor.blue)/newRadius);
    }
    else color = outFadeColor;
    return color;
}
