                                 ...
                                 int tolerance, int frameFreq)
{
     ...
     */
    solidColorPicker newColor(fillColor);
    return filler::fill<Stack>(img, x, y, newColor, tolerance, frameFreq);
}
                                ...
                                int gridSpacing, int tolerance, int frameFreq)
{
     ...
     * correct call to fill with the correct colorPicker parameter.
     */
    gridColorPicker newColor(gridColor, gridSpacing);
    return filler::fill<Stack>(img, x, y, newColor, tolerance, frameFreq);
}
                                    ...
                                    int radius, int tolerance, int frameFreq)
{
     ...
     * correct call to fill with the correct colorPicker parameter.
     */
    gradientColorPicker newColor(fadeColor1, fadeColor2, radius, x, y);
    return filler::fill<Stack>(img, x, y, newColor, tolerance, frameFreq);
}
                                 ...
                                 int tolerance, int frameFreq)
{
     ...
     */
    solidColorPicker newColor(fillColor);
    return filler::fill<Queue>(img, x, y, newColor, tolerance, frameFreq);
}
                       ...
                       int tolerance, int frameFreq)
{
    ...
    xcoords.add(x);
    ycoords.add(y);
    RGBAPixel originalPixel = (*img(x,y));
    animation final;
    int currFrames = 0;

    while(!xcoords.isEmpty() && !ycoords.isEmpty())
    {
        size_t currX=xcoords.remove();
        size_t currY=ycoords.remove();
        int minTolerance = pow((img(currX,currY)->red - originalPixel.red), 2) + pow((img(currX,currY)->green - originalPixel.green), 2) + pow((img(currX,currY)->blue - originalPixel.blue), 2);

        if(minTolerance<=tolerance && processed[currX][currY]==0)
        {
            currFrames++;
